...
export default function RestaurantPage() {
  ...
  const [worthPrice, setWorthPrice] = useState(null)
  const [bringVisitor, setBringVisitor] = useState(null)
  ...
  return (
    <main style={{minHeight:'100vh',background:'#F7F3EE',fontFamily:'sans-serif',paddingBottom:'48px'}}>
      ...
      <div style={{margin:'16px',background:'linear-gradient(135deg,#3D2B4F,#6B4E8A)',borderRadius:'16px',padding:'24px',color:'#F7F3EE'}}>
        ...
        <div style={{fontSize:'13px',opacity:0.7}}>{restaurant.cuisine} · {restaurant.neighbourhood} · {'£'.repeat(restaurant.price_range)}</div>
      </div>
      ...
      <div style={{padding:'0 16px 16px'}}>
        ...
        <div style={{background:'white',borderRadius:'16px',padding:'4px 0',boxShadow:'0 2px 12px rgba(26,23,20,0.06)'}}>
          {[
            {label:'Worth the price?',value:worthPrice,setter:setWorthPrice,opts:[{v:true,l:'Yes'},{v:false,l:'No'}]},
            {label:'Bring a visitor?',value:bringVisitor,setter:setBringVisitor,opts:[{v:'yes',l:'Yes'},{v:'local',l:'Locals only'}]},
            {label:'Better than expected?',value:betterThanExpected,setter:setBetterThanExpected,opts:[{v:true,l:'Yes'},{v:false,l:'No'}]},
          ].map((q,i) => (
            <div key={i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',borderBottom:i<2?'1px solid #EDE8E1':'none'}}>
              <span style={{fontSize:'14px',color:'#1A1714'}}>{q.label}</span>
              <div style={{display:'flex',gap:'6px'}}>
                {q.opts.map(opt => (<button key={opt.l} onClick={() => q.setter(opt.v)} style={{padding:'6px 14px',borderRadius:'8px',border:q.value===opt.v?'1.5px solid #2A6B4F':'1.5px solid #DDD6CC',background:q.value===opt.v?'#DCF0E6':'transparent',color:q.value===opt.v?'#2A6B4F':'#5A534E',fontSize:'13px',fontFamily:'sans-serif',cursor:'pointer',fontWeight:q.value===opt.v?'500':'400'}}>{opt.l}</button>))}
              </div>
            </div>
          ))}
        </div>
